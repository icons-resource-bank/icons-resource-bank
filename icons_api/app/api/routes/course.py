...
async def create_course(request: AuthedRequest, data: CourseCreateRequest):
...
@limiter.limit("10/5 seconds")
@auth_check
async def get_course(request: AuthedRequest, id: str):
...
@limiter.limit("5/5 seconds")
@flag_check(admin=True)
async def update_course(request: AuthedRequest, id: str, data: CourseUpdateRequest):
    course = await request.app.state.courses.get_course(id=id)
    if not course:
        raise CustomValidationError("Course not found", 404)
...
@limiter.limit("5/5 seconds")
@flag_check(admin=True)
async def delete_course(request: AuthedRequest, id: str):
...
@limiter.limit("10/5 seconds")
@auth_check
async def get_tags(request: AuthedRequest):
...
@limiter.limit("5/5 seconds")
@flag_check(admin=True)
async def create_tag(request: AuthedRequest, data: TagCreateRequest):
...
@limiter.limit("10/5 seconds")
@auth_check
async def get_tag(request: AuthedRequest, id: str):
...
@limiter.limit("5/5 seconds")
@flag_check(admin=True)
async def update_tag(request: AuthedRequest, id: str, data: TagUpdateRequest):
    tag = await request.app.state.courses.get_tag(id=id)
    if not tag:
        raise CustomValidationError("Tag not found", 404)
...
@limiter.limit("5/5 seconds")
@flag_check(admin=True)
async def delete_tag(request: AuthedRequest, id: str):
